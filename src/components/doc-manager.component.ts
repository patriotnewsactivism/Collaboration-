            ...
            @if (collab.userRole() === 'host') {
              <div class="bg-gray-900 rounded-lg p-3 border border-gray-800">
                ...
                <input 
                  ...
                  class="w-full bg-gray-800 border border-gray-700 rounded p-3 md:p-2 text-base md:text-sm text-white mb-3 md:mb-2 placeholder-gray-500 focus:border-blue-500 focus:outline-none"
                />
                <textarea 
                ...
                <button 
                  ...
                  class="mt-3 md:mt-2 w-full py-3 md:py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm md:text-xs font-bold rounded transition-colors disabled:opacity-50 touch-manipulation">
                  + Add Document
                </button>
              </div>
            }
             ...
             @if (collab.userRole() === 'viewer') {
               <div class="bg-gray-900 rounded-lg p-3 border border-purple-900/50">
                 ...
                 <textarea 
                   ...
                   rows="3"
                   class="w-full bg-gray-800 border border-gray-700 rounded p-3 md:p-2 text-base md:text-sm text-white placeholder-gray-500 focus:border-purple-500 focus:outline-none resize-none"
                 ></textarea>
                 <button 
                   (click)="submitSuggestion()"
                   [disabled]="!suggestionText"
                   class="mt-3 md:mt-2 w-full py-3 md:py-2 bg-purple-600 hover:bg-purple-500 text-white text-sm md:text-xs font-bold rounded transition-colors disabled:opacity-50 touch-manipulation">
                   Submit Suggestion
                 </button>
               </div>
             }
                 ...
                 @if (collab.userRole() === 'host' && sug.status === 'pending') {
                   <div class="flex gap-2">
                     <button (click)="collab.updateSuggestionStatus(sug.id, 'accepted')" 
                             class="flex-1 py-3 md:py-1 bg-green-900 hover:bg-green-800 text-green-300 text-xs rounded border border-green-800 transition-colors touch-manipulation">
                       Accept
                     </button>
                     <button (click)="collab.updateSuggestionStatus(sug.id, 'rejected')" 
                             class="flex-1 py-3 md:py-1 bg-red-900 hover:bg-red-800 text-red-300 text-xs rounded border border-red-800 transition-colors touch-manipulation">
                       Reject
                     </button>
                   </div>
                 }
